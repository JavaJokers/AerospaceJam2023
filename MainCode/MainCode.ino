...
void loop()
{
	...
	{
		Serial.println("Incoming Bluetooth command...") char data = Bluetooth.read();
		String sData = String(data);
		Serial.println(sData);
		// If query is for lidar (letter 'l'), return lidar stats
		if (sData == "l")
		{
			fetchLidar();
		...
		// Elif query is for light wavelength sensor (letter 'c'), return light stats
		else if (sData == "c")
		{
		...
		{ // DirtGrabber Close (Letter "s")
			Serial.println("dirtgrabber close");
			servo.attach(servo_pin);
			for (pos = 165; pos >= 50; pos -= 1)
		...
		{
			// IR box open
			Serial.println(F("Sending IR signal on Samsung protocol"));
    		IrSender.sendSamsung(0x707, 0x60, 1); // Offset: 0x707, Command: 0x60 (Up), Repeat once
		}
		else if (sData == "p") {
			// IR box close
			Serial.println(F("Sending IR signal on Samsung protocol"));
    		IrSender.sendSamsung(0x707, 0x61, 1);  // Offset: 0x707, Command: 0x61 (Down), Repeat once
		}
	}
}

void fetchLidar()
{ // TODO: if check fails, retry up to 10 times
	Lidar.listen();
	delay(75);
